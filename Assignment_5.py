# -*- coding: utf-8 -*-
"""
Created on Wed Feb 28 13:29:28 2024
...
A successful run of the test cases does not guarantee accuracy or fulfillment of
the requirements. Please do not submit your work if test cases fail.
3. To run unit tests simply use the below command after filling in all of the code:
python 05_assignment.py
4. Unless explicitly stated, please do not import any additional libraries but feel
free to use built-in Python packages
...
The data can be arbitrary integers. For example
    ...
    import pandas as pd
    import numpy as np
    numlist = {'0':[1, 2, 3 ], '0':[1, 2, 3, 4],'1': [5, 6, 7, 8],'2':[7, 8, 9, 10]}
    df = pd.DataFrame(data=numlist)
    print(df)
...
def exercise02(a):
    # The function exercise02() receives a Python list and convertsit to an ndarray. Convert the list to a numpy ndarray called array."""
...
#The function exercise03() receives an ndarray of integers. Return the sum of those integers using NumPy.
    ...
    import pandas as pd
    import numpy as np
    ndarray = np.array([[10, 15],[18, 36],[5,19]])
    sum = np.sum(ndarray)
    print(sum)
...
def exercise07(sd,m,s):
    ...
    sd = np.std(random_numbers)
    m = np.mean(random_numbers)
    s = np.size(random_numbers)
    np.allclose(sd, m)
...
def exercise10(n):
    ...
    # ------ Place code below here \/ \/ \/ ------
    identity_matrix = np.identity(dimension, dtype= float)
    # ------ Place code above here /\ /\ /\ ------  
...
def exercise11(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    array_1d = np.array([[1, 3, 5, 7],[2, 4, 6, 8]])
    array_reshaped = array_1d.shape(3,6)
    # ------ Place code above here /\ /\ /\ ------  
...
def exercise12(n):
    ...
    A checkerboard matrix n is a matrix with alternating 1s and 0s across rows and columns with the top left value equal to 1
    '''
    # ------ Place code below here \/ \/ \/ ------
    checkerboard_matrix = np.zeros((n, n), dtype=int)
...
def exercise13(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    vlist = [1, 20, 13, np.nan, 0, 1, 5, 23] 
  
# making series from list 
    series = pd.Series(vlist) 
  
# calling method 
    s = series.s(skipna = False) 
...
def exercise14(words):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    df = pd.DataFrame({'words':["hello, car, bye, machine, Victor, spyder"]}) 
