...
of the 'data' column values.
    ...
    # ------ Place code below here \/ \/ \/ ------
    import pandas as pd
    df = pd.DataFrame({'data' : [34, 45, 12],'city':['New York','Chicago', 'Boston']})
    print("Original DF:\n", df,"\n")
...
'B', 'C'. Return the DataFrame.
    ...
    # ------ Place code below here \/ \/ \/ ------
    import pandas as pd
    excel_url = "https://github.com/vitugo23/DATA602/blob/main/data.xls.xlsx"
    df6 = pd.read_excel(excel_url)
    print(df6)
    # ------ Place code above here /\ /\ /\ ------
    return df6
...
return the average value of that column.
    ...
    # ------ Place code below here \/ \/ \/ ------
    import pandas as pd
    df = pd.DataFrame({'column_name': [10, 20, 30], 'value': [20, 30, 40]})
    print(df)
    
    avg_value = df['column_name'].mean()
    print("average is :", avg_value)
